...
def backbone(protein_filename: str, chain="all chains", CA_only=True):
    # Uses a PDB file to get the coordinates of atoms in the backbone of a protein chain
    with open(protein_filename) as file:
        pdb_data = file.read()
    atoms = []
    backbone_atoms = ['CA'] if CA_only else ['CA', 'C', 'N']
    for line in pdb_data.split('\n'):
        # By default, look at all chains, unless just one is specified
        if line[0:4] == 'ATOM' and (chain == "all chains" or line[21] == chain) and line[13:16].rstrip(' ') in backbone_atoms:
